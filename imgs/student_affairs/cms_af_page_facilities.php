...
<body class="container py-5">
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if ($facility['image']): ?>
                                        <img src="<?= str_starts_with($facility['image'], 'http') ? $facility['image'] : '../../' . $facility['image'] ?>" alt="Facility Image" style="max-width: 100px;">
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <button class="btn btn-sm btn-primary" onclick="editFacility(<?= $facility['id'] ?>)">Edit</button>
                                    <form method="POST" class="d-inline">
                                        <input type="hidden" name="facility_id" value="<?= $facility['id'] ?>">
                                        <button type="submit" name="delete_facility" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this facility?')">Delete</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                ...
                <div class="modal-body">
                    <form action="" method="POST" enctype="multipart/form-data" id="addFacilityForm">
                        ...
                        <div class="mb-3">
                            ...
                            <?= renderEditor('operating_hours', '', 'operating_hours') ?>
                        </div>
                ...
                <div class="modal-body">
                    <form method="POST" id="editForm">
                        ...
                        <div class="mb-3">
                            ...
                            <?= renderEditor('edit_description', '', 'description') ?>
                        </div>

                        <div class="mb-3">
                            <label for="edit_operating_hours" class="form-label">Operating Hours</label>
                            <?= renderEditor('edit_operating_hours', '', 'operating_hours') ?>
                        </div>
    ...
    <script>
        function editFacility(id) {
            const facility = <?= json_encode($facilities) ?>.find(f => f.id === id);
            if (facility) {
                document.getElementById('edit_facility_id').value = facility.id;
                document.getElementById('edit_title').value = facility.title;
                document.getElementById('edit_description').innerHTML = facility.description;
                document.getElementById('edit_operating_hours').innerHTML = facility.operating_hours;
        ...
        function submitForm(formId) {
            ...
            if (formId === 'addFacilityForm') {
                ...
                document.getElementById('operating_hours').innerHTML = document.getElementById('operating_hours').value;
            } else if (formId === 'editForm') {
                document.getElementById('edit_description').innerHTML = document.getElementById('edit_description').value;
                document.getElementById('edit_operating_hours').innerHTML = document.getElementById('edit_operating_hours').value;
            }
