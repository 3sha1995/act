...
class ObjectiveFunctionCMS {
    ...
    private function ensureTablesExist() {
        try {
            ...
            $this->pdo->exec("CREATE TABLE IF NOT EXISTS health_obj (
            ...
            $this->pdo->exec("CREATE TABLE IF NOT EXISTS health_funct (
                id INT PRIMARY KEY DEFAULT 1,
            ...
            $stmt = $this->pdo->query("SELECT COUNT(*) FROM health_obj");
            if ($stmt->fetchColumn() == 0) {
                $this->pdo->exec("INSERT INTO health_obj (id, section_title, description, is_visible) 
                                VALUES (1, 'OUR OBJECTIVES', '', 1)");
            }

            // Insert default row in functions table if empty
            $stmt = $this->pdo->query("SELECT COUNT(*) FROM health_funct");
            if ($stmt->fetchColumn() == 0) {
                $this->pdo->exec("INSERT INTO health_funct (id, section_title, description, is_visible) 
                                VALUES (1, 'KEY FUNCTIONS', '', 1)");
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update_obj'])) {
        $cms->updateSection('health_obj', $_POST['obj_title'], $_POST['obj_desc'], isset($_POST['obj_visible']) ? 1 : 0);
    }

    if (isset($_POST['update_funct'])) {
        $cms->updateSection('health_funct', $_POST['funct_title'], $_POST['funct_desc'], isset($_POST['funct_visible']) ? 1 : 0);
    }
}

// Fetch existing content
$objData = $cms->getSection('health_obj');
$functData = $cms->getSection('health_funct');
?>
