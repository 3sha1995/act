...
class ObjectiveFunctionCMS {
    ...
    private function ensureTablesExist() {
        try {
            ...
            $this->pdo->exec("CREATE TABLE IF NOT EXISTS career_obj (
            ...
            $this->pdo->exec("CREATE TABLE IF NOT EXISTS career_funct (
                id INT PRIMARY KEY DEFAULT 1,
            ...
            $stmt = $this->pdo->query("SELECT COUNT(*) FROM career_obj");
            if ($stmt->fetchColumn() == 0) {
                $this->pdo->exec("INSERT INTO career_obj (id, section_title, description, is_visible) 
                                VALUES (1, 'OUR OBJECTIVES', '', 0)");
            }

            // Insert default row in functions table if empty
            $stmt = $this->pdo->query("SELECT COUNT(*) FROM career_funct");
            if ($stmt->fetchColumn() == 0) {
                $this->pdo->exec("INSERT INTO career_funct (id, section_title, description, is_visible) 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update_obj'])) {
        $cms->updateSection('career_obj', $_POST['obj_title'], $_POST['obj_desc'], isset($_POST['obj_visible']) ? 1 : 0);
    }

    if (isset($_POST['update_funct'])) {
        $cms->updateSection('career_funct', $_POST['funct_title'], $_POST['funct_desc'], isset($_POST['funct_visible']) ? 1 : 0);
    }
}

// Fetch existing content
$objData = $cms->getSection('career_obj');
$functData = $cms->getSection('career_funct');
?>
