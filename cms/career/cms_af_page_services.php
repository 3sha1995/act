...
class ServicesCMS {
    ...
    private function ensureTablesExist() {
        try {
            ...
            $sql = "CREATE TABLE IF NOT EXISTS career_services (
            ...
            $sql = "CREATE TABLE IF NOT EXISTS career_services_main (
                id int(11) NOT NULL AUTO_INCREMENT,
            ...
            $stmt = $this->pdo->query("SELECT COUNT(*) FROM career_services_main");
            if ($stmt->fetchColumn() == 0) {
                $sql = "INSERT INTO career_services_main (section_title, section_description, is_visible) 
    ...
    public function getMainSection() {
        try {
            ...
            $stmt = $this->pdo->query("SELECT * FROM career_services_main WHERE id = 1");
            $result = $stmt->fetch(PDO::FETCH_ASSOC);

            if (!$result) {
                return [
                    'section_title' => 'Our Services',
                    'section_description' => 'We provide a range of services to support our university community.',
                    'is_visible' => 1
                ];
            }
    ...
    public function updateMainSection($title, $description, $isVisible) {
        $stmt = $this->pdo->prepare("UPDATE career_services_main 
    ...
    public function getAllServices($page = 1, $perPage = 6) {
        try {
            // Get total count
            $countStmt = $this->pdo->query("SELECT COUNT(*) FROM career_services");
            ...
            $stmt = $this->pdo->prepare("SELECT * FROM career_services ORDER BY created_at DESC LIMIT :limit OFFSET :offset");
    ...
    public function addService($data) {
        try {
            ...
            $stmt = $this->pdo->prepare("INSERT INTO career_services (service_title, service_description, icon_path, is_visible)
    ...
    public function updateService($id, $data) {
        try {
            // Get current icon
            $stmt = $this->pdo->prepare("SELECT icon_path FROM career_services WHERE id = ?");
            $stmt->execute([$id]);
            ...
            $stmt = $this->pdo->prepare("UPDATE career_services 
    ...
    public function deleteService($id) {
        $stmt = $this->pdo->prepare("DELETE FROM career_services WHERE id = ?");
...
<?php foreach ($services as $service): ?>
    ...
    <script>
        ...
        function updateEditIconPreview(input) {
            const preview = document.getElementById('edit-icon-preview_icon');
            const imgPreview = document.getElementById('edit-icon-preview_img');
