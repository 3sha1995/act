...
class CMS_AboutSection {
    ...
    private function createDefaultContent() {
        try {
            $sql = "INSERT INTO `career_about` (`id`, `ontop_title`, `main_title`, `image_path`, `description`, `is_visible`) 
                    VALUES (1, 'ABOUT US', 'CAREER GUIDANCE', '../imgs/cte.jpg', 
...
class CMS_Dashboard {
    ...
    public function getStats() {
        try {
            $stats = [
                'services' => $this->countRows('career_services'),
                'officers' => $this->countRows('career_officers'),
                'activities' => $this->countRows('career_activities'),
                'facilities' => $this->countRows('career_facilities'),
                'processes' => $this->countRows('career_processes')
            ];
            return $stats;
        } catch (PDOException $e) {
...
<body>
    ...
    ?>
        ...
        <div class="current-image-container">
            ...
            <?php if (is_array($content) && !empty($content['image_path'])): ?>
        ...
        <div class="editor-toolbar">
            ...
            <div class="editor" id="editor" contenteditable="true"><?= is_array($content) ? ($content['description'] ?? '') : '' ?></div>
        <input type="hidden" name="description" id="description">
            ...
            <div class="form-group">
                <label class="switch-label">
                    Visibility:
                    <label class="switch">
                        <input type="checkbox" id="is_visible" name="is_visible" <?= (is_array($content) && isset($content['is_visible']) && $content['is_visible'] == 1) ? 'checked' : '' ?>>
