...
class MissionVisionCMS {
    ...
    private function ensureTablesExist() {
        try {
            ...
            $this->pdo->exec("CREATE TABLE IF NOT EXISTS career_mv (
            ...
            $stmt = $this->pdo->query("SELECT COUNT(*) FROM career_mv");
            if ($stmt->fetchColumn() == 0) {
                $this->pdo->exec("INSERT INTO career_mv (
    ...
    public function getMV() {
        try {
            $sql = "SELECT * FROM career_mv LIMIT 1";
    ...
    public function updateMV($data) {
        try {
            ...
            try {
                $checkColumn = $this->pdo->query("SHOW COLUMNS FROM career_mv LIKE 'updated_at'");
                if ($checkColumn->rowCount() === 0) {
                    // Column doesn't exist, add it
                    error_log("Adding missing updated_at column to career_mv table");
                    $this->pdo->exec("ALTER TABLE career_mv ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP");
                }
            } catch (PDOException $e) {
