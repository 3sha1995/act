...
class ProcessInfoCMS {
    ...
    private function ensureTableExists() {
        try {
            $sql = "CREATE TABLE IF NOT EXISTS guidance_process_info (
            ...
            $stmt = $this->conn->query("SELECT COUNT(*) FROM guidance_process_info");
            if ($stmt->fetchColumn() == 0) {
                $sql = "INSERT INTO guidance_process_info (section_title, section_description, is_visible) 
                        VALUES ('OUR OBJECTIVES', 
                                'Default guidance process information and objectives.',
                                1)";
                $this->conn->exec($sql);
            }
    ...
    public function getInfo() {
        $stmt = $this->conn->prepare("SELECT * FROM guidance_process_info LIMIT 1");
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }

    public function updateInfo($title, $description, $is_visible) {
        $stmt = $this->conn->prepare("UPDATE guidance_process_info 
            SET section_title = :title, section_description = :description, is_visible = :visible 
            WHERE id = 1");
        return $stmt->execute([
            ':title' => $title,
