...
class FacilitiesCMS {
    ...
    private function ensureTableExists() {
        try {
            $sql = "CREATE TABLE IF NOT EXISTS guidance_facilities (
            ...
            $sql = "CREATE TABLE IF NOT EXISTS guidance_facilities_settings (
                id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    private function ensureMainTitleExists() {
        try {
            $stmt = $this->pdo->query("SELECT COUNT(*) FROM guidance_facilities_settings");
            if ($stmt->fetchColumn() == 0) {
                $sql = "INSERT INTO guidance_facilities_settings (main_title, section_visible) VALUES ('OUR FACILITIES', 1)";
                $this->pdo->exec($sql);
            }
        } catch (PDOException $e) {
    ...
    public function getSettings() {
        try {
            $stmt = $this->pdo->query("SELECT * FROM guidance_facilities_settings WHERE id = 1");
    ...
    public function updateSettings($mainTitle, $sectionVisible) {
        try {
            $sql = "UPDATE guidance_facilities_settings SET main_title = :main_title, section_visible = :section_visible WHERE id = 1";
    ...
    public function getAllFacilities($page = 1) {
        try {
            ...
            $stmt = $this->pdo->prepare("SELECT * FROM guidance_facilities ORDER BY id DESC LIMIT :limit OFFSET :offset");
    ...
    public function getTotalPages() {
        try {
            $stmt = $this->pdo->query("SELECT COUNT(*) FROM guidance_facilities");
    ...
    public function getFacilityById($id) {
        try {
            $stmt = $this->pdo->prepare("SELECT * FROM guidance_facilities WHERE id = ?");
            $stmt->execute([$id]);
            return $stmt->fetch(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
    ...
    public function addFacility($data) {
        try {
            $sql = "INSERT INTO guidance_facilities (title, description, image, operating_hours, is_visible) 
    ...
    public function updateFacility($id, $data) {
        try {
            $sql = "UPDATE guidance_facilities SET 
                    title = :title,
    ...
    public function deleteFacility($id) {
        try {
            $stmt = $this->pdo->prepare("DELETE FROM guidance_facilities WHERE id = ?");
