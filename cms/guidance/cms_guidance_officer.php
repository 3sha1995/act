...
try {
    ...
    $createTableSQL = "CREATE TABLE IF NOT EXISTS `guidance_officer` (
...
class OfficerCMS {
    ...
    public function getAllOfficers() {
        try {
            $stmt = $this->pdo->query("SELECT * FROM guidance_officer ORDER BY id DESC");
    ...
    public function getSectionTitle() {
        try {
            $stmt = $this->pdo->query("SELECT section_title FROM guidance_officer ORDER BY id DESC LIMIT 1");
    ...
    public function addOfficer($name, $position, $imageUrl, $isVisible = 1) {
        try {
            $stmt = $this->pdo->prepare("INSERT INTO guidance_officer (name, position, image_url, is_visible) VALUES (?, ?, ?, ?)");
    ...
    public function updateOfficer($id, $name, $position, $imageUrl, $isVisible) {
        try {
            $stmt = $this->pdo->prepare("UPDATE guidance_officer SET name = ?, position = ?, image_url = ?, is_visible = ? WHERE id = ?");
    ...
    public function deleteOfficer($id) {
        try {
            ...
            $stmt = $this->pdo->prepare("SELECT image_url FROM guidance_officer WHERE id = ?");
            $stmt->execute([$id]);
            $officer = $stmt->fetch();

            // Delete the record
            $stmt = $this->pdo->prepare("DELETE FROM guidance_officer WHERE id = ?");
    ...
    public function toggleVisibility($id, $visible) {
        try {
            $stmt = $this->pdo->prepare("UPDATE guidance_officer SET is_visible = ? WHERE id = ?");
    ...
    public function updateSectionTitle($newTitle) {
        try {
            $stmt = $this->pdo->prepare("UPDATE guidance_officer SET section_title = ?");
    ...
    public function getSectionVisibility() {
        try {
            $stmt = $this->pdo->query("SELECT is_visible FROM guidance_officer LIMIT 1");
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            return $result ? $result['is_visible'] : 1;
        } catch (PDOException $e) {
    ...
    public function updateSectionVisibility($visible) {
        try {
            $stmt = $this->pdo->prepare("UPDATE guidance_officer SET is_visible = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_title'])) {
        ...
        if (isset($_POST['toggle_section'])) {
            $isVisible = $_POST['is_visible'] ? 1 : 0;
...
<head>
    ...
    <style>
        ...
        /* Update the modal image preview styles */
        .modal .image-preview {
            max-width: 200px;
            max-height: 200px;
            object-fit: cover;
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin: 20px 0;">
        ...
        <form method="POST" style="margin: 0; padding: 0; background: none; box-shadow: none; display: flex; align-items: center; gap: 10px;">
            <label class="switch-label">
                Section Visibility:
                <div class="switch">
                    <input type="checkbox" name="is_visible" <?= $sectionVisible ? 'checked' : '' ?> onchange="this.form.submit()">
