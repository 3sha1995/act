...
try {
    ...
    // Fetch about content from career_about table
    ...
    $stmt = $pdo->prepare("SELECT * FROM career_mv WHERE id = 1");
    $stmt->execute();
    $mvContent = $stmt->fetch(PDO::FETCH_ASSOC);
    
    // Fetch Objectives content
    $stmt = $pdo->prepare("SELECT * FROM career_obj WHERE id = 1");
    $stmt->execute();
    $objectivesContent = $stmt->fetch(PDO::FETCH_ASSOC);

    // Fetch Process Info content
    $stmt = $pdo->prepare("SELECT * FROM career_process_info WHERE id = 1");
    $stmt->execute();
    $processInfo = $stmt->fetch(PDO::FETCH_ASSOC);

    // Fetch Functions section
    $stmt = $pdo->prepare("SELECT * FROM career_funct WHERE id = 1");
    $stmt->execute();
    ...
    <div class="student_affair_about_divider"></div>
        ...
        <div class="student_affairs_about_content scroll-fade">
            <div class="student_affairs_about_image">
                <img src="<?php echo htmlspecialchars($aboutContent['image_path']); ?>" alt="Career Guidance Center">
            </div>
